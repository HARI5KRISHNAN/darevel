...
const ThreadListItem: React.FC<ThreadListItemProps> = ({ thread, isSelected, onSelect, isPinned, onTogglePin, onStarToggle, onMarkAsUnread, selectedFolder }) => {
    ...
    // Hide pin, unread, and star icons in draft, spam, and trash folders
    const isRestrictedFolder = ['draft', 'spam', 'trash'].includes(selectedFolder);
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-1">
                        <div className="flex items-center gap-2 truncate">
                            {!isRestrictedFolder && isThreadUnread && <div className="w-2 h-2 bg-blue-600 rounded-full flex-shrink-0"></div>}
                            {/* REFINEMENT: Use semibold for unread threads for a cleaner look. */}
                            <h3 className={`${!isRestrictedFolder && isThreadUnread ? 'font-semibold' : 'font-medium'} text-sm text-slate-800 truncate`}>
                                {latestMessage.sender}
                                {threadCount > 1 && <span className="font-normal text-slate-500 ml-1">({threadCount})</span>}
                            </h3>
                            {!isRestrictedFolder && isPinned && <PinIcon className="w-4 h-4 text-blue-600 flex-shrink-0" isFilled={true} />}
                        </div>
                        <div className="flex-shrink-0 ml-2">
                             <p className={`text-xs text-slate-500 ${!isRestrictedFolder ? 'group-hover:hidden' : ''}`}>{timeAgo(latestMessage.timestamp)}</p>
                             {!isRestrictedFolder && (
                    ...
                    <p className={`${!isRestrictedFolder && isThreadUnread ? 'font-semibold text-slate-900' : 'font-medium text-slate-600'} text-sm truncate`}>{latestMessage.subject.replace(/Re: /g, '')}</p>
