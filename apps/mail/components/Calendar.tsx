...
interface Meeting {
  ...
  organizer: string;
  attendees: string[];
  source?: 'mail' | 'chat'; // Track where meeting came from
}

// Transform Mail backend meeting to frontend format
function transformMailMeeting(backend: BackendMeeting): Meeting {
  const startTime = new Date(backend.scheduled_at);
  const endTime = new Date(startTime.getTime() + (backend.duration || 60) * 60 * 1000);

  return {
    id: `mail_${backend.id}`,
    title: backend.title || 'Untitled Meeting',
    description: backend.description || '',
    start_time: startTime.toISOString(),
    end_time: endTime.toISOString(),
    location: 'Online Meeting',
    meeting_link: backend.room_name ? `https://meet.jit.si/${backend.room_name}` : undefined,
    organizer: backend.organizer_email || 'Unknown',
    attendees: [],
    source: 'mail'
  };
}

// Transform Chat backend meeting to frontend format
function transformChatMeeting(backend: ChatMeeting): Meeting {
  return {
    id: `chat_${backend.id}`,
    title: backend.title || 'Untitled Meeting',
    description: backend.description || '',
    start_time: backend.startTime,
    end_time: backend.endTime,
    location: 'Online Meeting',
    meeting_link: backend.meetingLink,
    organizer: backend.organizerName || 'Chat User',
    attendees: [],
    source: 'chat'
  };
}
...
const Calendar: React.FC<CalendarProps> = ({ onJoinMeeting, refreshTrigger }) => {
  ...
  const fetchMeetings = async () => {
    try {
      ...
      // Fetch from both Mail and Chat backends in parallel
      const now = new Date();
      const threeMonthsLater = new Date();
      threeMonthsLater.setMonth(threeMonthsLater.getMonth() + 3);

      const [mailResponse, chatResponse] = await Promise.allSettled([
        api.get('/mail/calendar/meetings'),
        fetch(`${CHAT_API_URL}/api/meetings/range?startDate=${now.toISOString()}&endDate=${threeMonthsLater.toISOString()}`)
      ]);

      let allMeetings: Meeting[] = [];

      // Process Mail meetings
      if (mailResponse.status === 'fulfilled') {
        const backendMeetings: BackendMeeting[] = mailResponse.value.data.meetings || [];
        allMeetings = [...allMeetings, ...backendMeetings.map(transformMailMeeting)];
      } else {
        console.warn('Failed to fetch Mail meetings:', mailResponse.reason);
      }

      // Process Chat meetings
      if (chatResponse.status === 'fulfilled' && chatResponse.value.ok) {
        const chatMeetings: ChatMeeting[] = await chatResponse.value.json();
        allMeetings = [...allMeetings, ...chatMeetings.map(transformChatMeeting)];
      } else {
        console.warn('Failed to fetch Chat meetings');
      }
