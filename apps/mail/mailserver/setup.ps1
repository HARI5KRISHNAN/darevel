# Darevel Mail Server Setup Script (PowerShell)
# This script initializes the mail server configuration on Windows

$ErrorActionPreference = "Stop"

Write-Host "===================================" -ForegroundColor Cyan
Write-Host "Darevel Mail Server Setup" -ForegroundColor Cyan
Write-Host "===================================" -ForegroundColor Cyan
Write-Host ""
...
$postfixRunning = docker ps | Select-String "darevel-postfix"
...
$dovecotRunning = docker ps | Select-String "darevel-dovecot"
if ($dovecotRunning) {
...
try {
    docker exec darevel-postfix postmap /etc/postfix/vmailbox | Out-Null
...
# Reload Postfix
docker exec darevel-postfix postfix reload 2>&1 | Out-Null
...
try {
    docker exec darevel-postfix sh -c $createDirsCmd 2>&1 | Out-Null
...
$virtDomains = docker exec darevel-postfix postconf -h virtual_mailbox_domains
if ($virtDomains -match "darevel.local") {
...
$virtMaps = docker exec darevel-postfix postconf -h virtual_mailbox_maps
...
$mailboxDirs = docker exec darevel-postfix sh -c "ls -1 /var/mail/vhosts/darevel.local/ 2>/dev/null | wc -l"
...
docker exec darevel-postfix sh -c $testEmailCmd 2>&1 | Out-Null
Start-Sleep -Seconds 2

# Check if email was delivered
$newMails = docker exec darevel-dovecot sh -c "ls /var/mail/vhosts/darevel.local/bob/new/ 2>/dev/null"
if ($newMails) {
...
Write-Host "  - Domain: darevel.local"
...
Write-Host "docker exec darevel-postfix mailq" -ForegroundColor White
Write-Host "  Check mailbox:    " -NoNewline
Write-Host "docker exec darevel-dovecot ls -la /var/mail/vhosts/darevel.local/bob/new/" -ForegroundColor White
Write-Host "  Watch logs:       " -NoNewline
Write-Host "docker logs -f darevel-postfix" -ForegroundColor White
Write-Host ""

Write-Host "Send test email:" -ForegroundColor Yellow
Write-Host "  docker exec darevel-postfix sh -c `"echo 'Subject: Test' | sendmail bob@darevel.local`"" -ForegroundColor White
Write-Host ""
