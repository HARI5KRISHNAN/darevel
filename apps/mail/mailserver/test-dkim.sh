...
docker exec darevel-postfix postconf | grep -E "^(smtpd_milters|non_smtpd_milters|milter_default_action)"
echo ""

# Check OpenDKIM connection
echo "3. Testing Postfix -> OpenDKIM connectivity..."
docker exec darevel-postfix nc -zv opendkim 8891
echo ""

# Send test email via SMTP
echo "4. Sending test email with DKIM signature..."
docker exec darevel-postfix sh -c 'echo -e "Subject: DKIM Test Email\nFrom: test@darevel.local\nTo: test@darevel.local\n\nThis is a test email with DKIM signature.\n\nIf you see DKIM-Signature in the headers, it worked!" | sendmail -v test@darevel.local'
echo ""

echo "5. Checking Postfix logs for DKIM activity..."
sleep 2
docker logs darevel-postfix --tail 20 | grep -i "milter\|dkim" || echo "No DKIM/milter activity in recent logs"
echo ""

echo "6. Checking OpenDKIM logs..."
docker logs darevel-opendkim --tail 10
echo ""
