...
import { ChevronLeftIcon, PencilIcon, ShareIcon, ClipboardListIcon, ChatBubbleBottomCenterTextIcon, PaperClipIcon, UsersIcon, CheckCircleIcon, DocumentTextIcon, ArrowDownTrayIcon, PlusIcon, ArrowUturnLeftIcon, ClockIcon, TrashIcon, RefreshIcon } from './icons';
...
const TaskListItem: React.FC<{ task: ProjectTask; onEdit: (task: ProjectTask) => void; onToggleComplete: (taskId: string) => void; }> = ({ task, onEdit, onToggleComplete }) => {
    ...
    return (
        <div className="flex items-center gap-3 p-2 rounded-md hover:bg-background-main group/taskitem">
            <button onClick={() => onToggleComplete(task.id)} className="hover:opacity-80 transition-opacity">
...
const ProjectDetailView: React.FC<ProjectDetailViewProps> = ({ project, currentUser, onBack, onEditProject, onDeleteProject, onUpdateProject, onRefreshProject }) => {
    ...
    const [comments, setComments] = useState<ProjectComment[]>(Array.isArray(project.comments) ? project.comments : []);
    const [replyingTo, setReplyingTo] = useState<ProjectComment | null>(null);
    const [newCommentText, setNewCommentText] = useState('');
    const [files, setFiles] = useState<ProjectFile[]>(project.files || projectFiles);
    ...
    useEffect(() => {
        ...
        setFiles(project.files || projectFiles);
    }, [project]);

    // Sync all changes with parent project when they change (debounced to avoid loops)
    useEffect(() => {
        const timeoutId = setTimeout(() => {
            onUpdateProject(project.id, { tasks, comments, files });
        }, 300);
    ...
    const handleRefresh = async () => {
        if (!onRefreshProject || isRefreshing) return;
    ...
    // Immediately save tasks to backend (bypasses debounce for critical operations)
    const saveTasksImmediately = (newTasks: ProjectTask[]) => {
        onUpdateProject(project.id, { tasks: newTasks, comments, files });
    };
    ...
    const handleSaveTask = (taskData: {
        ...
        saveTasksImmediately(updatedTasks); // Save immediately to backend
        handleCloseModal();
    };
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        Array.from(uploadedFiles).forEach(file => {
            const newFile: ProjectFile = {
                ...
                type: file.type.includes('pdf') ? 'PDF' : file.type.includes('image') ? 'Image' : file.type.includes('word') ? 'Word' : 'Generic',
    ...
    <>
        ...
        <header className="flex-shrink-0 mb-6 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
                ...
                <button onClick={() => onDeleteProject(project.id)} className="bg-status-red text-white px-4 py-2 rounded-lg text-sm font-semibold hover:opacity-90 transition-opacity flex items-center gap-2">
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <DetailCard title="Tasks" icon={<ClipboardListIcon className="w-5 h-5 text-text-secondary" />} action={
                    ...
                    {filteredTasks.length > 0 ? (
                        filteredTasks.map(task => <TaskListItem key={task.id} task={task} onEdit={handleOpenEditModal} onToggleComplete={handleToggleTaskComplete} />)
                    ) : (
                ...
                <DetailCard title="Discussion" icon={<ChatBubbleBottomCenterTextIcon className="w-5 h-5 text-text-secondary" />}>
                    ...
                    <form onSubmit={handleCommentSubmit} className="mt-4 flex items-start gap-3">
                        <img src={currentUser.avatar} alt={currentUser.name} className="w-8 h-8 rounded-full object-cover shrink-0 mt-1.5" />
                ...
                {/* Files */}
                <DetailCard title="Files" icon={<PaperClipIcon className="w-5 h-5 text-text-secondary" />} action={
                    <>
                        <input
                            ref={fileInputRef}
                            type="file"
                            ...
                            id="file-upload"
                        />
                        <label htmlFor="file-upload" className="text-sm font-semibold text-accent hover:underline flex items-center gap-1 cursor-pointer">
