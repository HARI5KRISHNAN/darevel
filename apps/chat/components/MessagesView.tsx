...
import VideoCall from '../../shared/video/components/VideoCall';
...
const MessagesView: React.FC<MessagesViewProps> = ({ user, searchQuery }) => {
    ...
    // Jitsi call state
    const [activeCall, setActiveCall] = useState<{ roomName: string; callType: 'audio' | 'video'; jwtToken?: string } | null>(null);
    ...
    // Jitsi room name generator - use simple alphanumeric format
    const generateRoomName = (channelId: string) => {
        // Generate a random alphanumeric string for public room access
        const randomId = Math.random().toString(36).substring(2, 15);
        const timestamp = Date.now().toString(36);
        return `call${randomId}${timestamp}`;
    };
    ...
    const handleWebSocketMessage = useCallback((message: Message) => {
        devLog('📩 Received message via WebSocket:', message);

        // Check if this is a call notification message
        if (message.content && message.content.includes('🎥') && message.content.includes('started a')) {
            // Extract room name and call type from message
            const callMatch = message.content.match(/Join: ([a-zA-Z0-9-]+)/);
            const isAudioCall = message.content.includes('audio call');
            const isVideoCall = message.content.includes('video call');
            
            if (callMatch && (isAudioCall || isVideoCall) && user && message.userId !== user.id) {
                const roomName = callMatch[1];
                const callType = isAudioCall ? 'audio' : 'video';
                const callerName = message.userName || 'Someone';
                
                devLog('📞 Incoming call detected:', { roomName, callType, callerName, channelId: message.channelId });
                ...
                setTimeout(() => {
                    setIncomingCall(prev => {
                        if (prev && prev.roomName === roomName) {
                            return null;
                        }
                        return prev;
                    });
        ...
        setConversations(prev => {
            ...
            if (existingConvo) {
                ...
                devLog('🧾 Conversations updated (existing):', updated.map(c => ({ id: c.id, unreadCount: c.unreadCount, lastMessage: c.lastMessage })));
                return updated;
            } else {
                ...
                if (message.sender && message.channelId) {
                    ...
                    if (message.channelId.startsWith('dm-')) {
                        ...
                        devLog('🧾 Conversations updated (created):', updated.map(c => ({ id: c.id, unreadCount: c.unreadCount, lastMessage: c.lastMessage })));
                        return updated;
                    }
                    // Handle group messages
                        ...
                        // Parse creation message: "🎉 Group \"Name\" created with X members: User1, User2, User3 [IDS:1,2,3]"
                        const groupCreationMatch = message.content.match(/Group "([^"]+)" created with \d+ members: (.+?) \[IDS:[0-9,]+\]/);
                        if (groupCreationMatch) {
                            groupName = groupCreationMatch[1];
                            const memberNamesStr = groupCreationMatch[2];
                            // Try to match members from available users
                            const memberNames = memberNamesStr.split(', ').map(n => n.trim());
                            members = availableUsers.filter(u => memberNames.includes(u.name));
                            // Add current user to members if their name is in the list
                        ...
                        if (!savedGroups.find((g: any) => g.id === message.channelId)) {
                            savedGroups.push({
                                id: newGroup.id,
                                name: newGroup.name,
                        ...
                        devLog(`✨ Auto-created group conversation: ${groupName} with ${members.length} members`);
    ...
    // Fetch all registered users when user logs in
    useEffect(() => {
        if (user) {
    ...
    useEffect(() => {
        ...
        const loadUserConversations = async () => {
            try {
                ...
                devLog(`Loading ${channelIds.length} conversations for user ${user.id}`);

                // Fetch all users to map channel IDs to user info
                const AUTH_API_URL = import.meta.env.VITE_AUTH_SERVICE_URL || 'http://localhost:8086';
                ...
                const savedGroups = JSON.parse(localStorage.getItem('userGroups') || '[]');
                const groupConversations = savedGroups.map((group: any) => ({
                    id: group.id,
                    name: group.name,
                    avatar: group.avatar,
                ...
                // Merge groups with DM conversations
                const allConversations = [...groupConversations, ...loadedConversations];

                // Apply saved pin/mute settings from localStorage
                const savedSettings = loadConversationSettings();
                const conversationsWithSettings = allConversations.map(convo => {
                ...
                devLog(`✓ Loaded ${conversationsWithSettings.length} conversations`);
            } catch (error) {
                devError('Error loading user conversations:', error);
            }
        };
    ...
    useEffect(() => {
        ...
        const fetchMessages = async () => {
            try {
                ...
                devLog(`✓ Loaded ${messages.length} messages for ${selectedConversationId}`);
            } catch (error) {
    ...
    const fetchRegisteredUsers = async () => {
        try {
            devLog('👥 Fetching registered users...');
            ...
            devLog('👥 Parsed users:', users);
            // Filter out current user
            const otherUsers = Array.isArray(users) ? users.filter((u: User) => u.id !== user?.id) : [];
            devLog('👥 Available users (excluding current):', otherUsers.map(u => `${u.name}(${u.id})`));
    ...
    const handleCreateGroup = async () => {
        ...
        try {
            const allMembers = [user, ...selectedGroupMembers];
            const memberNames = allMembers.map(m => m.name).join(', ');
            const memberIds = allMembers.map(m => m.id).join(',');
            await apiSendMessage(
                groupId,
                `🎉 Group "${groupName}" created with ${allMembers.length} members: ${memberNames} [IDS:${memberIds}]`,
                user.id
            );
        } catch (error) {
            devError('Error sending group creation message:', error);
        }
    };

    // Update group name
    const handleUpdateGroupName = (conversationId: string, newName: string) => {
        setConversations(prev => 
            prev.map(convo => {
                if (convo.id === conversationId && convo.isGroup) {
                    return {
                        ...convo,
                        name: newName,
        ...
        const updatedGroups = savedGroups.map((group: any) => 
            group.id === conversationId ? { ...group, name: newName, avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(newName)}&background=random` } : group
        );
        localStorage.setItem('userGroups', JSON.stringify(updatedGroups));
    };

    // Add members to group
    const handleAddGroupMembers = async (conversationId: string, newMembers: User[]) => {
        if (!user) return;
        
        let updatedMembersList: User[] = [];
        setConversations(prev => 
            prev.map(convo => {
                if (convo.id === conversationId && convo.isGroup) {
                    updatedMembersList = [...(convo.members || []), ...newMembers];
                    return {
                        ...convo,
        ...
        const savedGroups = JSON.parse(localStorage.getItem('userGroups') || '[]');
        const updatedGroups = savedGroups.map((group: any) => 
            group.id === conversationId ? { ...group, members: updatedMembersList } : group
        );
        localStorage.setItem('userGroups', JSON.stringify(updatedGroups));

        // Send notification message about new members
        try {
            const memberNames = newMembers.map(m => m.name).join(', ');
            const conversation = conversations.find(c => c.id === conversationId);
            await apiSendMessage(
                conversationId,
                `✨ ${user.name} added ${memberNames} to the group "${conversation?.name}"`,
                user.id
            );
        } catch (error) {
            devError('Error sending member addition message:', error);
        }
    };
    ...
    const handleGenerateSummary = async (messagesToSummarize?: Message[]) => {
        ...
        try {
            ...
            devError("Error generating summary:", error);
    ...
    // Handle starting a call with Jitsi
    const handleStartCall = async (callType: 'audio' | 'video') => {
        if (!selectedConversation || !user) return;

        // Generate unique room name for this conversation
        const roomName = generateRoomName(selectedConversation.id);

        devLog('📞 Starting Jitsi call:', { roomName, callType, isGroup: selectedConversation.isGroup });
        ...
        try {
            ...
            if (keycloakToken) {
                ...
                if (response.success && response.token) {
                    ...
                    devLog('✅ Jitsi JWT token received');
                }
            }
        } catch (error) {
            devError('Error fetching Jitsi JWT:', error);
        }

        // Open Jitsi call
        setActiveCall({ roomName, callType, jwtToken });

        // Send notification message to the conversation
        try {
            const callTypeText = callType === 'audio' ? 'audio call' : 'video call';
            const memberCount = selectedConversation.isGroup && selectedConversation.members
                ? ` (${selectedConversation.members.length} members)`
                : '';

            await apiSendMessage(
                selectedConversation.id,
                `🎥 ${user.name} started a ${callTypeText}${memberCount}. Join: ${roomName}`,
                user.id
            );
        } catch (error) {
            devError('Error sending call notification:', error);
        }
    };

    // Handle closing Jitsi call
    ...
    const handleAcceptCall = async () => {
        ...
        devLog('✅ Accepting call:', incomingCall);
        ...
        try {
            ...
            if (keycloakToken) {
                const meetingService = new MeetingService();
                meetingService.setAuthToken(keycloakToken);
                const response = await meetingService.getJitsiToken(incomingCall.roomName);
                if (response.success && response.token) {
                    jwtToken = response.token;
                    devLog('✅ Jitsi JWT token received for incoming call');
                }
            }
        } catch (error) {
            devError('Error fetching Jitsi JWT for incoming call:', error);
        }
    ...
    const handleDeclineCall = () => {
        ...
        devLog('❌ Declining call:', incomingCall);
    ...
    return (
        <div className="flex-1 flex min-w-0 h-full overflow-hidden">
            ...
            <aside className={`bg-background-panel flex flex-col border-r border-border-color transition-all duration-300 ease-in-out ${isCollapsed ? 'w-20' : 'w-96'} h-full`}>
            ...
            <main className="flex-1 flex flex-col bg-background-main min-w-0 h-full overflow-hidden">
            ...
            {incomingCall && !activeCall && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                    <div className="bg-background-panel border border-border-color rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4 animate-[slideUp_0.3s_ease-out]">
                        {/* Call Icon */}
                        <div className="flex justify-center mb-6">
                            <div className={`w-20 h-20 rounded-full flex items-center justify-center animate-pulse ${
                                incomingCall.callType === 'audio' ? 'bg-green-500/20' : 'bg-blue-500/20'
                            }`}>
                        ...
                        <div className="text-center mb-8">
                            {(() => {
                                const conversation = conversations.find(c => c.id === incomingCall.channelId);
                                const isGroupCall = conversation?.isGroup;
                                return (
                                    <>
                                        <h3 className="text-2xl font-bold text-text-primary mb-2">
                                            {isGroupCall ? conversation?.name : incomingCall.callerName}
                                        </h3>
                                        <p className="text-text-secondary">
                                            {isGroupCall 
                                                ? `${incomingCall.callerName} started a ${incomingCall.callType} call`
                                                : `Incoming ${incomingCall.callType} call...`
                                            }
                                        </p>
                                        {isGroupCall && conversation?.members && (
                                            <p className="text-xs text-text-secondary mt-1">
                                                {conversation.members.length} members in group
                                            </p>
                                        )}
                                    </>
                                );
                        ...
                        <div className="flex gap-4">
                            ...
                            <button
                                ...
                                className="flex-1 py-4 px-6 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl transition-all transform hover:scale-105 active:scale-95 flex items-center justify-center gap-2"
                            >
                            ...
                            <button
                                ...
                                className="flex-1 py-4 px-6 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl transition-all transform hover:scale-105 active:scale-95 flex items-center justify-center gap-2"
                            >
                                <span className="text-xl">✓</span>
                                <span>Accept</span>
                            </button>
            ...
            {activeCall && user && (
                <div className="fixed inset-0 z-50 bg-black">
                    <VideoCall
                        ...
                        startWithVideoMuted={activeCall.callType === 'audio'}
                        jwtToken={activeCall.jwtToken}
                    />
                </div>
            )}
        </div>
