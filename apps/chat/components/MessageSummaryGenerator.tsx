...
const MessageSummaryGenerator: React.FC<MessageSummaryGeneratorProps> = ({ messages }) => {
  ...
  const generateSummary = async () => {
    ...
    try {
      // Use AI Service endpoint (port 8087)
      const AI_API_URL = import.meta.env.VITE_AI_SERVICE_URL || 'http://localhost:8087';
      const response = await fetch(`${AI_API_URL}/api/ai/generate-summary`, {
        method: "POST",
