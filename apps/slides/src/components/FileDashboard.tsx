...
import { 
  ...
  Plus, Cloud, Image, Video, Music, File, Download, Grid, MoreVertical,
  ChevronRight, Folder, UploadCloud, Layout, FileText, MonitorPlay, 
  Briefcase, Table, FileDown
} from 'lucide-react';
...
const FileDashboard: React.FC<FileDashboardProps> = ({ onBack, onOpenTemplates }) => {
  return (
    <div className="fixed inset-0 bg-white dark:bg-[#0f172a] text-slate-800 dark:text-slate-300 z-50 flex font-sans transition-colors duration-200">
      ...
      <div className="w-64 bg-slate-50 dark:bg-[#0f172a] border-r border-slate-200 dark:border-slate-800 flex flex-col p-6">
        ...
        <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
          ...
          <button className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl flex items-center justify-center gap-2 font-medium shadow-lg shadow-blue-500/20 dark:shadow-blue-900/20 transition-all active:scale-95">
        ...
        <div className="flex-1 overflow-y-auto p-8">
           ...
           <div className="mb-10">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                 ...
                 <QuickItem icon={Table} label="Sheet" color="text-emerald-500 dark:text-emerald-400" bg="bg-emerald-100 dark:bg-emerald-500/10" />
           ...
           <div>
              <div className="flex items-center justify-between mb-6">
                 ...
                 <button className="text-sm text-blue-600 dark:text-blue-400 font-medium hover:text-blue-500 dark:hover:text-blue-300">See all</button>
              </div>
                 ...
                 <table className="w-full text-left text-sm">
                    <thead>
                       <tr className="text-slate-500 border-b border-slate-200 dark:border-slate-800">
                          ...
                          <th className="font-medium p-4">File Item</th>
                          <th className="font-medium p-4">Last Modified</th>
                          <th className="font-medium p-4 pr-6">File Size</th>
                       </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                       <FileRow name="All Ehem Collections" items="21 items" date="Jan 2, 2019" size="12 Gb" isFolder />
                       <FileRow name="Homework" items="152 items" date="Feb 1, 2020" size="152 Gb" isFolder active />
                       <FileRow name="User Interface Exploration" items="32 items" date="Jan 2, 2019" size="213 Mb" isFolder />
                       <FileRow name="Lihat kebunku.mp4" items="1 items" date="Jan 2, 2019" size="2.4 Gb" icon={Video} iconColor="text-red-500" />
                       <FileRow name="JVNA - Catch Me.mp3" items="1 items" date="Jan 2, 2019" size="8 Mb" icon={Music} iconColor="text-amber-500" />
                    </tbody>
                 </table>
              </div>
           </div>
...
const QuickItem = ({ icon: Icon, label, color, bg }: any) => (
  <button className="flex flex-col items-center justify-center p-4 rounded-2xl bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors gap-3 shadow-sm dark:shadow-none">
...
const FileRow = ({ name, items, date, size, isFolder, active, icon: Icon, iconColor }: any) => (
  <tr className={`transition-colors cursor-pointer group ${active ? 'bg-blue-50 dark:bg-blue-500/5' : 'hover:bg-slate-50 dark:hover:bg-slate-800/30'}`}>
     <td className="p-4 pl-6 flex items-center gap-3">
        <div className={`p-2 rounded-lg ${isFolder ? 'bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400' : 'bg-slate-100 dark:bg-slate-800 ' + iconColor}`}>
           {isFolder ? <Folder size={18} fill="currentColor" fillOpacity={0.2} /> : <Icon size={18} />}
        </div>
        <span className={`font-medium ${active ? 'text-blue-600 dark:text-blue-400' : 'text-slate-700 dark:text-slate-200 group-hover:text-black dark:group-hover:text-white'}`}>{name}</span>
     </td>
     <td className="p-4 text-slate-500">{items}</td>
     <td className="p-4 text-slate-500">{date}</td>
     <td className="p-4 pr-6 text-slate-500 font-medium">{size}</td>
  </tr>
);
